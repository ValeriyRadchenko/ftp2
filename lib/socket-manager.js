...
class SocketManager extends EventEmitter {
    ...
    send(command, type) {
        return new Promise((resolve, reject) => {
            const socket = this.commandTLSSocket || this.commandNetSocket;
            console.log('command finish listener set');
    ...
    async createNetServer() {
        ...
        this.netServer = new NetServer(this.commandTLSSocket || this.commandNetSocket, this.logger);
        await this.netServer.start(this.options.active);
        return this.netServer;
    }
    ...
    activeServerFinish() {
        return new Promise(resolve => {
            if (!this.netServer) {
                console.log('activeServerFinish bad');
                resolve();
            }

            this.netServer.onTransferCompleteCommand()
                .then(result => {
                    console.log('activeServerFinish semi-bad');
                    if (!result) {
                        console.log('activeServerFinish good');
                        this.netServer = null;
                    }
                    resolve();
                });
    ...
    dataFinish() {
        return new Promise(resolve => {
            this.waitUntilDataWillTransferred()
                .then(() => {
                    return this.activeServerFinish();
                })
                .then(() => {
